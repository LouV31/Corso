...
const whatDayIsIt = function(){
  ...
  let giorni = new Array();
        ...
        giorni[5] = "Venerdì";
        giorni[6] = "Sabato";
  let giornoCorrente = giorni[today.getDay()];
...
let howManyDays = function(numeroData){ 
  const ora = Date.now();
  const newDate = new Date(numeroData);
  const differenza = (ora - newDate) / (1000 * 60 * 60 * 24);
  return parseInt(differenza);
}
...
const isTodayMyBirthday = function(){
  const myBirthDay = [31, 1];
  const day = new Date();
  let toDay = day.getDate();
  const thisMonth = day.getMonth();
  if (toDay === myBirthDay[0] && thisMonth === myBirthDay[1]){
    return true;
  } else {
    return false;
  }
...
console.log(deleteProp(testObj, "nome"));
/* ESERCIZIO 12
...
const onlyTheYears = (array) =>{
  return array.map(element => parseInt(element.Year));
}
...
const sumAllTheYears = (array) => {
  return array.reduce((acc, currentValue) => (acc) + (parseInt(currentValue.Year)), 0);
}
...
const emptyList = () => {
  const liItems = document.querySelectorAll("#myList li")
